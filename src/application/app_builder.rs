...
use esp_hal::peripherals::{UART0, GPIO9, LEDC};
...
impl<'a> AppBuilder<'a> {
        ...
        pub fn build(self) -> Result<Application, BuildError> {
            ...
            let fan: Box<dyn Fan + Send> = if let Some(fan) = self.fan {
                ...
                let fan_controller = FanController::with_ledc(ledc, gpio9)
            ...
            let heater = self.heater.ok_or(BuildError::MissingPeripheral("SSR Heater"))?;
            let bean_sensor = self.bean_sensor.ok_or(BuildError::MissingPeripheral("Bean Temperature Sensor"))?;
            let env_sensor = self.env_sensor.ok_or(BuildError::MissingPeripheral("Environment Temperature Sensor"))?;
            ...
            critical_section::with(|cs| {
                ...
                container.artisan_input.borrow(cs).borrow_mut().replace(artisan_input);
            });
    ...
    impl core::fmt::Display for BuildError {
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            match self {
                ...
                BuildError::MissingPeripheral(name) => write!(f, "Missing required peripheral: {}", name),
            }
        }
    }

...
mod target_impl {
    ...
    // The fixture surface is declared inline with only the fields the runner
    // touches (`name`, `reading`, `status_builder`, `expected_status_line`),
    // so the embedded (`--features embedded,regression`) build stays free of
    // host test-only deps. A future HIL-validated fixture set can be added
    ...
    impl OverTempTestRunner {
        ...
        pub async fn run_once(&mut self) {
            ...
            // If `emergency_shutdown` fails (heater/writer/etc. rejected the
            // off command), the heater and fan were just set to 100% and
            ...
            let shutdown_failed = ServiceContainer::with_roaster_async(|roaster| {
                ...
                // `emergency_shutdown` returns `Err(EmergencyShutdown)` BY
                // DESIGN (actuator.rs — it is the "emergency armed" signal,
                // not a failure). The real failure indicator is the SSR
                // hardware status: after the actuator's retries, `Error`
                // means the heater did NOT shut off.
            ...
            if shutdown_failed {
                ...
                // The emergency latch must NOT be cleared on this path.
                // `shutdown_failed` means the regression could not turn the
                // heater OFF (SSR stuck in `Error` after the retries) — a
                // real hardware fault. Keeping the latch (heater output
                // forced to 0, fan pinned at 100 %, commands rejected) is the
                // correct end state until the operator intervenes physically;
                // clearing it would drop the forced cooldown fan exactly when
                // the heater may be wedged on.
            ...
            // Feed the watchdog for 400 ms. The software watchdog timeout is
            // 1000 ms (`WATCHDOG_TIMEOUT_MS` in watchdog.rs — three real
            ...
            // Do not announce a regression pass with an empty catalogue — that
            // advertishes a regression that tested nothing. An empty gate is
            // left in deliberately for a future HIL-validated fixture set;
            // until then we emit an explicit EMPTY marker so the host cannot
            // mistake `SAFETY OT-REGRESSION` for a green run.
            let mut safety = String::<TRACE_EVENT_MAX_LEN>::new();
            ...
            // Restore operator control at the end of the run.
            // `emergency_shutdown("Over-temp regression")` above armed the
            // safety latch (`activate_emergency` + `fault_condition` +
        ...
        async fn replay_fixture(&mut self, fixture: &'static fixture_catalog::RegressionFixture) {
            ...
            // `SensorConversionHub::new()` (no args) only exists on the host
            // build; on `riscv32 + !simulated-sensors` the two-arg variant
            // applies. `from_fixture` exists exactly for this path and
            // internally uses `new_uninit()` — which the `regression`
            // feature makes select the safe `simulated-sensors` branch (see

...
impl ActuatorController {
    ...
    pub fn apply_guarded_heater(
        ...
        // NaN/±Inf must never reach the clamp/slew/status logic.
        // `NaN.clamp(0.0, 100.0)` passes NaN through, the slew computes NaN,
        // and `status.ssr_output = NaN` disarms the comms-idle and
    ...
    ///
    /// The retry discipline is shared by every emergency path (internal
    /// traps, manual Artisan STOP, safety policy outcomes).
    pub fn force_heater_off(&mut self, status: &mut SystemStatus) -> bool {
        ...
        // Resync the slew limiter. `force_heater_off` writes 0 % straight
        // through the SSR driver, bypassing `apply_guarded_heater`'s
    ...
    /// physical state rather than claiming full cooling while the fan never moved.
    ...
    pub fn emergency_shutdown(
        ...
        // `ssr_output` is NOT zeroed unconditionally. If the heater is
        // physically stuck ON (`force_heater_off` failed every attempt),
        // telemetry must not claim 0 % — the honest signal is
        // `ssr_hardware_status = Error`. `ssr_output` only drops to 0 once
        ...
        // Internal-trap emergencies (overtemp, NaN, RoR, watchdog) escalate a
        // total fan failure instead of absorbing it silently. Same rule as
        // `stop_streaming` and the command paths: no fan at 100 % means
        // unsafe to continue, so escalate with the fan-failure variant.
        if !fan_ok {
            log::error!(
    ...
    pub fn periodic_health_check(&mut self, now: Instant) {
        // Consecutive samples are one control-loop tick apart (~330 ms →
        // ~130 ms of PWM phase separation at 5 Hz), inside the (100, 200) ms
        // band that `heat_presence`'s debounce relies on. A GPIO read per
        // tick is negligible; this keeps the documented cadence.

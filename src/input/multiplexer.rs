...
// The `Instant` mock below is gated to unit tests only; device builds and
// host non-test builds use the real `embassy_time::Instant` so the 60 s
// idle-failover follows wall-clock time.
...
impl CommandMultiplexer {
    ...
    pub fn on_command_received(&mut self, channel: CommChannel) -> bool {
        ...
        match self.active_channel {
            CommChannel::None => {
                // The first syntactically-valid command on EITHER wire claims
                // the session (e.g. UART line noise that assembles `READ\r`
                // before Artisan connects over USB takes the session until
                // the 60 s idle reset). Alternatives were rejected as
                // regressions:
                ...
                //     grace window.
                // Existing mitigations: only VALID commands reach this
                // match — parse garbage is dropped before the gate; the
                // pre-parse gate also prevents parser-side FIFO
    ...
    /// Pure predicate — would a command on `channel` be accepted, WITHOUT
    /// activating the session? Unlike `should_process_command` /
    /// `on_command_received` (which activate from `None`), this NEVER mutates
    /// state. Transport layers use it to skip PARSING — and with it the
    /// parser-side PROFILE/FANPROFILE FIFO side effects (a dropped/refused
    /// command would otherwise leak its profile into the FIFO where a later
    /// session's `SetProfile` consumes it) — for lines the multiplexer gate
    /// would refuse anyway. Activation stays reserved for successfully parsed
    /// commands, so a garbage line can never claim the session through this path.

...
/// A SINGLE worker thread sleeps until the earliest deadline and fires all
/// due wakers (mirroring embassy-time's own `driver_std.rs`);
/// `schedule_wake` only enqueues (deduping by `waker.will_wake` so repeated
/// polls of the same timer do not grow the queue) and notifies the worker.
/// This matters because `embassy_time::Timer::poll` calls `schedule_wake` on
/// EVERY poll of a pending timer, so any `select(Timer, channel)` with
/// sustained traffic would otherwise re-schedule on every wake.
fn worker_loop() {
    loop {
...
impl Driver for HostTimeDriver {
    ...
    fn schedule_wake(&self, at: u64, waker: &Waker) {
        // Fire synchronously when the deadline has already arrived (the timer
        // is past expiry, the work should run now); otherwise enqueue on the
        // shared worker thread. Per the embassy-time `Driver` contract, for
        // a future deadline the waker must be re-invoked once the deadline
        // arrives (`Waker: Clone + Send + 'static`, so cloning it into the
        // queue is sound).
        let now = self.now();
        if at <= now {
            waker.wake_by_ref();
...
mod tests {
    ...
    /// Regression test for the host embassy-time driver.
    ///
    /// This test runs a `Timer::after` with NO other concurrent activity and
    /// asserts it completes — it validates that a future deadline enqueued
    /// on the shared worker fires without requiring concurrent work to
    ...
    fn schedule_wake_future_deadline_wakes_without_concurrent_activity() {
        ...
        // actively polling. The shared worker fires the waker after the
        // delay.

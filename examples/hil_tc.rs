...
fn main() -> ! {
    ...
    let bt_reading = match bt_sensor.read_raw_temperature() {
        Ok(reading) => {
            println!("TEST:tc_raw_02_conversion:PASS:raw=0x{:06X}", reading.raw_temp);
            reading
        }
        Err(_) => {
    ...
    if (0.0..=50.0).contains(&bt_temp) {
        ...
        println!("TEST:tc_raw_03_ambient:FAIL:temp={:.1}:reason=out_of_range", bt_temp);
    }
    ...
    if et_temp > 0.0 && final_bt_temp > 0.0 {
        println!("TEST:tc_raw_04_dual_channel:PASS:et={:.1},bt={:.1}", et_temp, final_bt_temp);
    } else {
        println!("TEST:tc_raw_04_dual_channel:FAIL:et={:.1},bt={:.1}:reason=invalid_temperatures", et_temp, final_bt_temp);
    }
    
    if bt_reading.fault & 0x1F == 0 {
        println!("TEST:tc_raw_05_no_fault:PASS:fault_register=0x{:02X}", bt_reading.fault);
    } else {
        println!("TEST:tc_raw_05_no_fault:FAIL:fault_register=0x{:02X}:reason=fault_detected", bt_reading.fault);
    }
    ...
    for i in 0..3 {
        match bt_sensor.read_raw_temperature() {
            ...
            Err(_) => {
                println!("TEST:tc_raw_06_stability:FAIL:reason=reading_error_iteration_{}", i + 1);
                loop {}
            }
        }
    ...
    if variance < 5.0 {
        println!("TEST:tc_raw_06_stability:PASS:et_variance={:.1},bt_variance={:.1}", 0.0, variance);
    } else {

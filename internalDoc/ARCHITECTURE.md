...
| `usb_queue_processor_task` | Process USB command queue | On data available |
| `queue_processor_task` | Process UART command queue | On data available |
| `dual_output_task` | Route output to active channel, write responses | Every 5ms |
| `control_loop_task` | Process commands, update sensors, control outputs | Every 100ms |
| `regression_task` | Over‑temperature regression testing (optional) | On `REG` command or watchdog trigger |
...
│                    (accumulate bytes until CR)                    │
└───────────────────────────┬─────────────────────────────────────┘
                            │ Raw command bytes
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│      usb_queue_processor_task / queue_processor_task              │
│                    (parse commands, enqueue)                      │
└───────────────────────────┬─────────────────────────────────────┘
                            │ ArtisanCommand
                            ▼
┌─────────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────────┐
│                    dual_output_task                                │
│              (route to active channel, add CRLF)                  │
└───────────────────────────┬─────────────────────────────────────┘
                            │ Serial Response
                            ▼
┌─────────────────────────────────────────────────────────────────┐
...
3. Queue processor task parses command and enqueues ArtisanCommand
4. Control loop receives and processes command from ArtisanChannel
5. Response formatted and sent to OutputChannel
6. Dual output task routes to active channel, adds CRLF
7. Response transmitted via USB CDC or UART0
```
...
- **Temperature limits**: Hard limit at 250°C with emergency shutdown at 260°C
- **Fault detection**: MAX31856 fault detection (open circuit, short, etc.)
- **Watchdog timer**: Hardware watchdog with automatic feed and failure telemetry via `STATUS` command
- **LEDC guard**: Prevents PWM runaway with timeout counters exposed in telemetry
- **Over-temperature regression testing**: `REG` command triggers 100% heater/fan test with safety logging
- **Command queue monitoring**: Queue depth metrics prevent command backlog saturation
- **Async lock instrumentation**: Concurrent access tracking prevents race conditions
...
**Architecture:** The UNITS command uses the `ManualCommandPolicy` trait pattern via `forward_artisan_manual_command()`. This centralizes manual command handling and ensures consistent policy enforcement across all manual commands (OT1, OT2, UP, DOWN, UNITS).
...
The STATUS command provides comprehensive telemetry including safety metrics and watchdog status. It returns 18 CSV fields:

1. **ET, BT, Heater, Fan** - Same as READ response for backward compatibility
2. **WatchdogOK** (0/1) - Watchdog feed success status
3. **WatchdogFailures** - Consecutive watchdog failure count
4. **LastWatchdogReason** - Failure reason token (or "none")
5. **LEDCGuardTimeouts** - LEDC guard timeout counter
6. **RegressionActive** (0/1) - Over‑temperature regression test active flag
7. **PV, MV, IntegratorValue, DerivativeValue** - PID controller internal state
8. **SaturationFlag, IntegratorClampFlag, DerivativeAvailableFlag** - PID saturation flags
9. **CommandLatency, MaxCommandLatency** - Command processing latency in microseconds

**Implementation:** The `ArtisanFormatter::format_status_response()` function formats the 18-field CSV from the `SystemStatus` struct defined in `src/config/constants.rs`. For complete field definitions and instrumentation usage, refer to [PROTOCOL.md](PROTOCOL.md).
